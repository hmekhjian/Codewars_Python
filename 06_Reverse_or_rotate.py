...
def revrot(strng, sz):
    if sz <= 0:
        ...
        subString = [strng[i : i + sz] for i in range(0, len(strng), sz)]
        newString = []
        newChunk = ""

        if len(subString[-1]) != sz:
            del subString[-1]

        for chunk in subString:
            sum = 0
            for digit in chunk:
            ...
            if sum % 2 == 0:
                ...
                newChunk = "".join(reversed(chunk))
                # print(newChunk)
                newString.append(newChunk)

            else:
                newChunk = chunk[1:] + chunk[0]
                # print(newChunk)
                newString.append(newChunk)
